...
- **x402 Protocol:** `GET /api/tasks/x402` returns HTTP 402 with payment headers. `POST /api/tasks/x402-execute` serves a request only when its `X-Payment` header (base64 of `{"payload": {"txHash": "0x...", "signature": "0x..."}}`) names a successful USDC transfer on Arc of at least the price to the payee, under 15 minutes old, that has not paid for a request before, and carries the paying wallet's signature of `AgentFlow x402 payment <txHash>`. The payment facts are read from the chain; nothing in the header is trusted but the hash and the signature. The signature is what stops someone who only saw the payment on the chain from claiming it first: an EOA signs with `personal_sign`, and a contract wallet such as a Circle smart account signs through ERC-1271.
