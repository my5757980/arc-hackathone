...
from ...blockchain.x402 import build_x402_header, consume_payment, verify_x402_payment, x402_payment_response
...
class TaskResponse(BaseModel):
    ...
    result: str
    cost_usdc: float
    tx_hash: Optional[str] = None  # None when no payment reached the chain
...
async def execute_task(request: TaskRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        tx = Transaction(
            ...
            status=task_result.payment_status,
            created_at=now,
        )
        db.add(tx)
...
async def x402_execute_task(
    ...
    check = await verify_x402_payment(x_payment, min_amount, orchestrator_address)
    if not check.ok:
        return await x402_payment_response(
            payee_address=orchestrator_address,
    ...
    # Spend the payment before doing the work, so the same transaction cannot pay twice.
    if not await consume_payment(db, check, request.task_type):
        return await x402_payment_response(
            payee_address=orchestrator_address,
            amount_usdc=min_amount,
            service=request.task_type,
            reason="this payment transaction has already been used",
        )
