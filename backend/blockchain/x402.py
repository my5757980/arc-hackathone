...
X-Payment header: base64 of JSON such as
    {"x402Version": 1, "scheme": "exact", "network": "arc-testnet", "payload": {"txHash": "0x..."}}
The server accepts it only when the Arc chain itself shows a successful, recent USDC transfer of at
least the price to the service's payTo address, and only once per transaction.
"""
import base64
import json
import os
...
def parse_payment_header(payment_header: str | None) -> str | None:
    ...
    tx_hash = (payload.get("txHash") if isinstance(payload, dict) else None) or payment.get("txHash")
...
async def verify_x402_payment(
    ...
    Nothing written in the header is trusted except the transaction hash: the payee, the amount and
    the success all come from the chain. This does not mark the payment as spent; call
    consume_payment() once it returns ok.
    """
...
async def x402_payment_response(
    ...
    content = {
        ...
        "message": (
            ...
            'then retry with header X-Payment: base64 of {"payload": {"txHash": "0x..."}}'
        ),
    }
    if reason:
